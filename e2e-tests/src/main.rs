...
async fn unlock_admin(runner: &TestRunner) -> Result<()> {
    ...
    if runner
        .driver
        .find_all(By::Css(r#"button[data-test="recipe-new-button"]"#))
        .await?
        .is_empty()
    {
        let unlock_btn = runner
            .driver
            .find(By::XPath(r#"//button[contains(., 'Unlock')]"#))
            .await
        ...
        for (idx, ch) in pin.chars().take(4).enumerate() {
            let input = runner
                .driver
                .find(By::Id(format!("pin-digit-{}", idx)))
                .await
...
async fn fill_first_ingredient(runner: &TestRunner) -> Result<()> {
    for _ in 0..10 {
        let select_el = runner
            .driver
            .find(By::Css(r#"select[data-test="recipe-ingredient-select-0"]"#))
            .await?;
        let select = SelectElement::new(&select_el).await?;
        ...
        if options.len() > 1 {
            ...
            let grams_input = runner
                .driver
                .find(By::Css(r#"input[data-test="recipe-ingredient-grams-0"]"#))
                .await?;
...
async fn create_recipe(runner: &TestRunner, name: &str) -> Result<()> {
    runner
        .driver
        .find(By::Css(r#"button[data-test="recipe-new-button"]"#))
        .await?
        .click()
        .await?;
    ...
    let name_input = runner
        .driver
        .find(By::Css(r#"input[data-test="recipe-name"]"#))
        .await?;
    ...
    let servings = runner
        .driver
        .find(By::Css(r#"input[data-test="recipe-servings"]"#))
        .await?;
    ...
    let tags = runner
        .driver
        .find(By::Css(r#"input[data-test="recipe-tags"]"#))
        .await?;
    tags.clear().await?;
    ...
    let instructions = runner
        .driver
        .find(By::Css(r#"textarea[data-test="recipe-instructions"]"#))
        .await?;
    ...
    runner
        .driver
        .find(By::Css(r#"button[data-test="recipe-save"]"#))
        .await?
        .click()
        .await?;
...
async fn edit_recipe(runner: &TestRunner, original: &str, updated: &str) -> Result<()> {
    ...
    runner
        .driver
        .find(By::Css(r#"button[data-test^="recipe-edit-"]"#))
        .await?
        .click()
        .await?;
...
async fn delete_recipe(runner: &TestRunner, name: &str) -> Result<()> {
    runner
        .driver
        .find(By::Css(r#"button[data-test^="recipe-delete-"]"#))
        .await?
        .click()
        .await?;

    // Confirm dialog
    let _ = runner.driver.accept_alert().await;
