...
# Find all relevant source files and generate a combined hash
{
  ...
  find crates/app/src crates/client/src crates/worker/src -type f -name "*.rs" 2>/dev/null | sort | xargs cat
  
  # Hash Cargo.toml files
  find crates -type f -name "Cargo.toml" 2>/dev/null | sort | xargs cat
