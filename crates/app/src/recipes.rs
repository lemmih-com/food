      ...
      <div class="rounded-lg border border-slate-200 bg-white p-6 shadow-md dark:border-slate-800 dark:bg-slate-900">
        <div class="mb-4 flex items-start justify-between">
          ...
          <Show when=move || is_authenticated.get()>
            <button
              class="p-1 text-blue-600 hover:text-blue-800 dark:text-blue-300 dark:hover:text-blue-200"
              title="Edit recipe"
              on:click={
        ...
        <Show when=move || has_ingredients>
          <div class="mb-4">
            ...
            <ul class="list-inside list-disc space-y-1 text-slate-700 dark:text-slate-200">
              {ingredients
                .iter()
                .map(|ing| {
        ...
        <div class="rounded border border-slate-200 bg-slate-50 p-3 dark:border-slate-800 dark:bg-slate-800/70">
          ...
          {move || {
            ...
            match (recipes_result, ingredients_result) {
                ...
                if recipes.is_empty() {
                  ...
                  view! {
                    <div class="py-12 text-center">
                      <p class="mb-4 text-slate-600 dark:text-slate-400">"No recipes yet."</p>
                      <Show when=move || auth.is_authenticated.get()>
              ...
              (Some(Err(e)), _) | (_, Some(Err(e))) => {
                view! {
                  <div class="rounded border border-red-200 bg-red-100 px-4 py-3 text-red-700 dark:border-red-500/40 dark:bg-red-900/40 dark:text-red-100">
              ...
              _ => view! { <p class="text-slate-600 dark:text-slate-400">"Loading..."</p> }.into_any(),
            }
          }}
        </Suspense>
      </div>
