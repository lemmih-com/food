...
const DEFAULT_SAT_FAT_PCT: f64 = 10.0; // 10% of daily calories
const DEFAULT_FIBER_MIN: i32 = 25;

// Local storage key
#[cfg(not(feature = "ssr"))]
const SETTINGS_STORAGE_KEY: &str = "food_settings";
...
impl Default for SettingsData {
    fn default() -> Self {
        let sat_fat_grams = (DEFAULT_SAT_FAT_PCT / 100.0 * DEFAULT_DAILY_CALORIES as f64
...
fn PresetButtons(
    ...
    let load_preset = move |preset: &str| {
        match preset {
            "default" => {
                // App defaults: protein 22%, carbs 43%, fat 35%, sat fat 10%
                let defaults = SettingsData::default();
                set_daily_calories.set(defaults.daily_calories);
                set_protein_pct.set(defaults.protein_pct);
                set_carbs_pct.set(defaults.carbs_pct);
                set_fat_pct.set(defaults.fat_pct);
                set_sodium_mg.set(defaults.sodium_mg);
                set_sat_fat_grams.set(defaults.sat_fat_grams);
                set_fiber_min.set(defaults.fiber_min);
            }
            "usda" => {
